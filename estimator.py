    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                if (str(body_part.get_part_name()).strip() not in ("CocoPart.LShoulder","CocoPart.LElbow","CocoPart.LWrist")):
                    continue
...
#                cv2.putText(npimg, center, center, 4, 2, 255, 3, 8)
                ...
                if (str(body_part.get_part_name()).strip()=="CocoPart.LShoulder"):
                    a = np.array([int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5)])
                
                if (str(body_part.get_part_name()).strip()=="CocoPart.LElbow"):
                    b = np.array([int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5)])
                    
                if (str(body_part.get_part_name()).strip()=="CocoPart.LWrist"):
